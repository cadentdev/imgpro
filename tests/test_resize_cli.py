...
def run_imagepro_resize(input_file, *args):
    ...
    """
    cmd = [sys.executable, 'imagepro.py', 'resize', '--input', str(input_file)] + list(args)
...
class TestResizeCommandBasics:
    ...
    def test_resize_command_exists(self):
        ...
        cmd = [sys.executable, 'imagepro.py', 'resize', '--help']
        result = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
    ...
    def test_resize_requires_input_argument(self):
        ...
        cmd = [sys.executable, 'imagepro.py', 'resize', '--width', '300']
        result = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
    ...
    def test_resize_requires_width_or_height(self):
        ...
        cmd = [sys.executable, 'imagepro.py', 'resize', '--input', str(img_path)]
        result = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
    ...
    def test_resize_file_not_found(self, temp_dir):
        ...
        exit_code, stdout, stderr = run_imagepro_resize(fake_file, '--width', '300')
...
class TestResizeByWidth:
    ...
    def test_resize_single_width(self, temp_dir):
        ...
        exit_code, stdout, stderr = run_imagepro_resize(
...
class TestResizeOutputHandling:
    ...
    def test_resize_default_output_directory(self, temp_dir):
        ...
        try:
            os.chdir(temp_dir)
            cmd = [sys.executable, str(Path(old_cwd) / 'imagepro.py'), 'resize',
...
class TestResizeExitCodes:
    ...
    def test_resize_success_exit_code(self, temp_dir):
        ...
        exit_code, stdout, stderr = run_imagepro_resize(img_path, '--width', '300')
        assert exit_code == 0
    ...
    def test_resize_unsupported_format_exit_code(self, temp_dir):
        ...
        exit_code, stdout, stderr = run_imagepro_resize(png_file, '--width', '300')
        assert exit_code == 1
