...
def run_imgpro_resize(input_file, *args):
    """
...
class TestResizeCommandBasics:
    ...
    def test_resize_command_exists(self):
        ...
        cmd = [sys.executable, 'imgpro.py', 'resize', '--help']
        result = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
    ...
    def test_resize_file_not_found(self, temp_dir):
        ...
        exit_code, stdout, stderr = run_imgpro_resize(fake_file, '--width', '300')
...
class TestResizeByWidth:
    ...
    def test_resize_single_width(self, temp_dir):
        ...
        exit_code, stdout, stderr = run_imgpro_resize(
...
class TestResizeExitCodes:
    ...
    def test_resize_success_exit_code(self, temp_dir):
        ...
        exit_code, stdout, stderr = run_imgpro_resize(img_path, '--width', '300')
        assert exit_code == 0
    ...
    def test_resize_unsupported_format_exit_code(self, temp_dir):
        ...
        exit_code, stdout, stderr = run_imgpro_resize(png_file, '--width', '300')
        assert exit_code == 1
